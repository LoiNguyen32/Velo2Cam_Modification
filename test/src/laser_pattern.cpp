...
//customer
vector<vector<Velodyne::Point*>> getR(pcl::PointCloud<Velodyne::Point> & pc)
{
  vector<vector<Velodyne::Point*>>  rings(32);
  for (pcl::PointCloud<Velodyne::Point>::iterator pt = pc.points.begin();pt <pc.points.end();pt++)
  {
...
void callback(const PointCloud2::ConstPtr& laser_cloud){
  ...
  //std::cout << "1xxxxxxxxxxxxxxx" << std::endl;
  pass2.setInputCloud (velo_filtered);
  pass2.setFilterFieldName ("z");
  pass2.setFilterLimits (-0.1, 0.6);
  pass2.filter (*velo_filtered);

  pass2.setInputCloud (velo_filtered);
  pass2.setFilterFieldName ("y");
  //pass2.setFilterLimits (-2, -1.55);
  pass2.setFilterLimits (-2.1, -1.4); 
  //pass2.setFilterLimits (-0.9, -0.4);
  ...
  //std::cout << "2xxxxxxxxxxxxxxx" << std::endl;
  ...
  //std::cout << "3xxxxxxxxxxxxxxx" << std::endl;
  if (inliers->indices.size () == 0)
  {
      std::cout << "[Laser] Could not estimate a planar model for the given dataset" << std::endl;
  ...
  //std::cout << "4xxxxxxxxxxxxxxx" << std::endl;

  // Get edges points by range
  //vector<vector<Velodyne::Point*> > rings = Velodyne::getRings(*velocloud);
  vector<vector<Velodyne::Point*> > rings = getR(*velocloud);
  //vector<vector<Velodyne::Point*> > rings = getR(*velo_filtered);

  std::cout << "5xxxxxxxxxxxxxxx" << std::endl;
  ...
  dit -> selectWithinDistance (coefficients_v, .05, inliers2); // 0.1 .05
  ...
  //vector<vector<Velodyne::Point*> > rings2 = Velodyne::getR(*pattern_cloud);
  vector<vector<Velodyne::Point*> > rings2 = getR(*pattern_cloud);
  int ringsWithCircle = 0;
    std::cout << "for5xxxxxxxxxxxxxxx" << std::endl;
  ...
  std::cout << "for5xxxxxxxxxxxxxxx" << std::endl;
    std::cout << ringsWithCircle << std::endl;
    std::cout << circles_cloud->points.size() << std::endl;
